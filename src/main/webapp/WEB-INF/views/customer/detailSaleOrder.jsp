...
<body>
	...
	<div class="title-cart" style="margin-top: 80px">
        <div class="history-buy" id="history"  onclick="hienHistory()">
            <h4 style="margin-left: 20px">Chi tiết đơn hàng</h4>
        </div>
    </div>
    
	<div style="margin-left: 250px; background-color: white;width: 980px;">
	<c:forEach items="${list_products}" var="a">
	<div class="card mb-3" style="max-width: 100%;">
		<div class="row g-0">
			<div class="col-md-4">
				<a href="${base }/chitietsanpham/${a.id}"><img class="img-fluid rounded-start" src="${base }/upload/${a.anh_san_pham}"></a>
			</div>
			<div class="col-md-8">
				<div class="card-body">
					<h5 class="card-title"><a style="text-decoration: none;color:black;" href="${base }/chitietsanpham/${a.id}">${a.ten_san_pham }</a></h5>
					<p class="card-text t">Giá tiền:<fmt:setLocale value="vi_VN"/>
                            <a style="text-decoration: none;color:black;" href="${base }/chitietsanpham/${a.id}">
                           <fmt:formatNumber value="${a.gia_ban }" type="currency"  />
                            </a></p>
					<p class="card-text">
						<small class="text-muted">Số lượng mua:${a.soLuongMua }</small>
					</p>
				</div>
			</div>
			<div style="left: 350px;position: relative;bottom: 40px;">
				<a href="${base }/chitietsanpham/${a.id}" class="btn btn-success">Xem sản phẩm</a>
					
				<button type="button" onclick="AddToCart('${base}', ${a.id }, 1)" class="btn btn-primary">Mua lại</button>
				</div>
		</div>
	</div>
	</c:forEach>
	...
	<script type="text/javascript">
		...
		$(document).ready(
				function() {
					var thanhTien = document
							.getElementsByClassName('thanhTien');
					var tong = 0;
					for (var i = 0; i < thanhTien.length; i++) {
						var tam = thanhTien[i].getAttribute('value').replace(
								".000 ₫", "000");
